...
#' @param excel_decomp Limits to 24 categories so it fits in the excel template for LE decomposition. You may need to recategorise breast or prostate cancer depending on sex. Defaults to true. The categories removed are epilepsy, bone cancer, sudden infant death syndrome, and other mental and behavioural conditions.
#' 
#' @return (dataframe) A dataframe with a new column containing the ICD 10 code names, called either `icd10_name_detailed` or `icd10_name_broad` depending on the value of `detailed`.
#' 
#' @examples 
...
#' 
#' @export
icd10_name <- function(df, code_column, detailed = TRUE, excel_decomp = TRUE) {
  code_column <- enquo(code_column)
  if (detailed && excel_decomp == FALSE) {
